...
${FAILED_SESSION_A}`);
  ...
  it("parses DeepSeek DSML format with spaces and special delimiters", () => {
    const dsml = `< calls> < | | DSML | | invoke name="read"> < | | DSML | | parameter name="filePath" string="true">Z:\\Hub\\projects\\ds2api\\config.json</ | | DSML | | parameter> </ | | DSML | | invoke> < | | DSML | | invoke name="read"> < | | DSML | | parameter name="filePath" string="true">Z:\\Hub\\projects\\ds2api\\set_token.mjs</ | | DSML | | parameter> </ | | DSML | | invoke> </ calls>`;
    const result = parseToolCalls(dsml, "test-dsml");

    expect(result.content).toBe("");
    expect(result.toolCalls).toHaveLength(2);
    expect(result.toolCalls[0]?.function).toEqual({
      name: "read",
      arguments: '{"filePath":"Z:\\\\Hub\\\\projects\\\\ds2api\\\\config.json"}',
    });
    expect(result.toolCalls[1]?.function).toEqual({
      name: "read",
      arguments: '{"filePath":"Z:\\\\Hub\\\\projects\\\\ds2api\\\\set_token.mjs"}',
    });
  });

  it("extracts DSML calls while preserving conversational prose", () => {
    const mixed = `Let me check the repository configuration first.\n<calls>< | | DSML | | invoke name="read">< | | DSML | | parameter name="filePath" string="true">config.json</ | | DSML | | parameter></ | | DSML | | invoke></calls>`;
    const result = parseToolCalls(mixed, "test-mixed");

    expect(result.content).toBe("Let me check the repository configuration first.");
    expect(result.toolCalls).toHaveLength(1);
    expect(result.toolCalls[0]?.function.name).toBe("read");
    expect(result.toolCalls[0]?.function.arguments).toBe('{"filePath":"config.json"}');
  });

  it("promotes DSML tool calls from reasoning without leaking raw markup", () => {
    const reasoning = `Thinking about files...\n<calls>< | | DSML | | invoke name="read">< | | DSML | | parameter name="filePath" string="true">Z:\\Hub\\projects\\ds2api\\config.json</ | | DSML | | parameter></ | | DSML | | invoke></calls>`;
    const result = parseToolCallsFromParts("", reasoning, "test-promote");
