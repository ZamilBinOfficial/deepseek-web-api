...
import { collectJsonObjects, looksLikeToolJson, repairJson } from "./toolCallJson.js";
...
const ALL_TOOL_TAGS = /<\s*\/?[|｜]?\s*(?:tool[_-]?call|_?call|calls|tool\s*calls?\s*(?:begin|end)?|(?:[|｜]\s*){0,2}DSML\s*(?:[|｜]\s*){0,2}(?:invoke|parameter))\b[^>]*>/gi;
...
function dsmlCandidates(text: string): { calls: PayloadCandidate[]; ranges: Range[] } {
  const calls: PayloadCandidate[] = [];
  const ranges: Range[] = [];

  const invokeRegex = /<\s*(?:[|｜]\s*){0,2}(?:DSML\s*(?:[|｜]\s*){0,2})?invoke\b(?=[^>]*\bname\s*=\s*["']([^"']+)["'])[^>]*>([\s\S]*?)<\s*\/\s*(?:[|｜]\s*){0,2}(?:DSML\s*(?:[|｜]\s*){0,2})?invoke\s*>/gi;
  const paramRegex = /<\s*(?:[|｜]\s*){0,2}(?:DSML\s*(?:[|｜]\s*){0,2})?parameter\b(?=[^>]*\bname\s*=\s*["']([^"']+)["'])([^>]*)>([\s\S]*?)<\s*\/\s*(?:[|｜]\s*){0,2}(?:DSML\s*(?:[|｜]\s*){0,2})?parameter\s*>/gi;
  ...
  while ((invokeMatch = invokeRegex.exec(text)) !== null) {
    ...
    const matchEnd = invokeMatch.index + invokeMatch[0].length;
    ...
    while ((paramMatch = paramRegex.exec(body)) !== null) {
      ...
      const rawVal = paramMatch[3] ?? "";

      if (!pname) continue;

      const isExplicitString = /string\s*=\s*["']true["']/i.test(attrs);
      if (isExplicitString) {
        args[pname] = rawVal.trim();
      } else {
        const trimmed = rawVal.trim();
        try {
          args[pname] = JSON.parse(trimmed);
        } catch {
          args[pname] = trimmed;
        }
      }
    ...
    if (!hasParams) {
      ...
      if (trimmedBody.startsWith("{")) {
        const parsedJson = repairJson(trimmedBody);
    ...
    calls.push({
      order: matchStart,
      consume: { start: matchStart, end: matchEnd },
      payload: { name, arguments: args },
    });
    ranges.push({ start: matchStart, end: matchEnd });
  }

  const callsTagRegex = /<\s*(\/?)\s*(?:[|｜]\s*){0,2}calls\b[^>]*>/gi;
  let callsTagMatch: RegExpExecArray | null;
  while ((callsTagMatch = callsTagRegex.exec(text)) !== null) {
    ranges.push({
      start: callsTagMatch.index,
      end: callsTagMatch.index + callsTagMatch[0].length,
    });
  }
...
function nativeV3Candidates(text: string): { calls: PayloadCandidate[]; ranges: Range[] } {
  const calls: PayloadCandidate[] = [];
  const ranges: Range[] = [];

  const callRegex = /<[|｜]?\s*tool\s*call\s*begin\s*[|｜]?>\s*(?:function\s*:\s*)?([a-zA-Z0-9_.-]+)\s*(?:```(?:json)?\s*)?([\s\S]*?)(?:```)?\s*<[|｜]?\s*tool\s*call\s*end\s*[|｜]?>/gi;
  ...
  while ((match = callRegex.exec(text)) !== null) {
    ...
    if (rawArgs) {
      const parsed = repairJson(rawArgs);
  ...
  const wrapperRegex = /<[|｜]?\s*tool\s*calls\s*(?:begin|end)\s*[|｜]?>/gi;
  let wrapMatch: RegExpExecArray | null;
  while ((wrapMatch = wrapperRegex.exec(text)) !== null) {
    ranges.push({
      start: wrapMatch.index,
      end: wrapMatch.index + wrapMatch[0].length,
    });
  }
...
function withoutToolTags(text: string): string {
  TOOL_TAG.lastIndex = 0;
  ALL_TOOL_TAGS.lastIndex = 0;
  return text.replace(ALL_TOOL_TAGS, "").replace(TOOL_TAG, "");
}
...
export function parseToolCalls(text: string, seed = "tool"): ParsedToolCalls {
  ...
  const dsml = dsmlCandidates(text);
  const nativeV3 = nativeV3Candidates(text);
  const allBlocked = [...blocks, ...dsml.ranges, ...nativeV3.ranges];
  const bareObjects = collectJsonObjects(text).filter(
    (object) => !overlaps({ start: object.start, end: object.end }, allBlocked),
  );
  const bareRanges = bareObjects
    .filter((object) => looksLikeToolJson(object.raw))
    .map((object) => ({ start: object.start, end: object.end }));
  const bareContext = protocolOnly(text, [...allBlocked, ...bareRanges]);
  ...
  const artifactRanges = [...tagged.artifacts, ...bareRanges, ...dsml.ranges, ...nativeV3.ranges];
  const isProtocolOnly = artifactRanges.length > 0 && protocolOnly(text, [...blocks, ...bareRanges, ...dsml.ranges, ...nativeV3.ranges]);
  if (toolCalls.length === 0 && !isProtocolOnly) return { content: text, toolCalls };
  const consumed = [
    ...candidates.map((candidate) => candidate.consume),
    ...dsml.ranges,
    ...nativeV3.ranges,
  ];
